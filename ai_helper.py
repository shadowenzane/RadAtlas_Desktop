...
# 心电图诊断查询 prompt
ECG_DIAGNOSIS_PROMPT = """你是一个资深心电诊断专家。根据以下心电图信息，给出最有可能的3-5条心电图诊断。
...
def test_llm_connection(provider, model, api_key, custom_url='', timeout=60):
    """测试LLM联通性
...
def _call_llm(api_url, api_key, model, messages, api_type='chat_completions', timeout=180):
    """调用单个LLM API，返回原始文本
...
def call_diagnosis_multi(exam_type, keywords, selected_providers=None, use_knowledge_base=None):
    ...
    def _query_one(provider_config):
        ...
        try:
            ...
            # 根据检查类型选择合适的prompt
            is_ecg = 'ECG' in exam_type or '心电图' in exam_type
            if is_ecg:
                prompt = ECG_DIAGNOSIS_PROMPT.format(keywords=keywords)
                system_role = '你是一个资深心电诊断专家。请始终以纯JSON数组格式回复，不要包含markdown代码块标记。'
            else:
                prompt = DIAGNOSIS_PROMPT.format(exam_type=exam_type, keywords=keywords)
                system_role = '你是一个资深医学影像诊断专家。请始终以纯JSON数组格式回复，不要包含markdown代码块标记。'
            messages = [
...
def _query_tencent_kb(kb_config, exam_type, keywords):
    ...
    def _ima_kb_search(query_text, kb_id=''):
        ...
        r.raise_for_status()
        d = r.json()
        # IMA API 返回字段为 code/msg，不是 error_code/error_msg
        if d.get('code', 0) != 0:
            raise ValueError(f'IMA知识库API错误 [{d.get("code")}]: {d.get("msg", "未知错误")}')
    ...
    if not knowledge_base_id:
        kb_error = '未配置 knowledge_base_id，知识库模块(wiki)不可用'
    else:
        try:
    ...
    if not doc_snapshots:
        ...
        def _ima_note_search(search_type, query_text, start=0, end=20):
            ...
            d = r.json()
            if d.get('code', 0) != 0:
                raise ValueError(f'IMA笔记API错误 [{d.get("code")}]: {d.get("msg", "未知错误")}')
    ...
    context = '\n\n'.join(context_parts)
    if not context:
        err_detail = f'（知识库模块: {kb_error}）' if kb_error else '（知识库和笔记模块均无结果）'
        raise ValueError(f'IMA知识库未找到与"{search_query}"相关的内容。{err_detail}')
...
def _query_volcengine_search_kb(kb_config, exam_type, keywords):
    ...
    try:
        ...
        for svc, reg in [('air', 'cn-beijing'), ('ark', 'cn-north-1')]:
            ...
            req.set_path(path)
            req.set_body(body)
            credentials = Credentials(access_key, secret_key, svc, reg)
            SignerV4.sign(req, credentials)
            sign_attempts.append((f'SignerV4({svc}/{reg})', dict(req.headers)))
    except ImportError:
        pass
